...
//    }
    ...
    @NotNull
    private static Map<String, T<?>> getKeysDescend(@NotNull final Object toDescend,
                                                     @NotNull final FlowMap o) {
        ...
        if(toDescend instanceof TemplateExpression) {
            ...
            ((Collection<?>) toDescend).forEach(element -> allKeys.putAll(getKeysDescend(element, o)));
        } else if (toDescend instanceof Map) {
            ((Map<?, ?>) toDescend).forEach((key, element) -> allKeys.putAll(getKeysDescend(element, o)));
        }
    ...
    @NotNull
    public static Map<String, T<?>> getKeysInTemplate(@NotNull T<?> t, @NotNull FlowMap o) {
        return getKeysDescend(t.getTerm(), o);
    }
}